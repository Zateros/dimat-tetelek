...
#align(center)[
  #cetz.canvas({
    ...
    content("x.mid", [$X$], anchor: "north", padding: .2)
    content("y.mid", [$Y$], anchor: "east", padding: .4)
    content("gr.north-east", [$X times Y$], anchor: "north-east", padding: -0.6)
...
+	_Mondja ki a gráf csúcsainak fokszáma és a gráf elszáma közötti osszefüggést! \ Van-e olyan egyszerű gráf, mely csúcsainak fokszámai `1,2,2,2,2,4`? \ Válaszát indokolja!_ \ \ Minden $G eq (V,E) "gráfra " limits(sum)_(v in V) d(v) eq 2 abs(E)$ \ \ Nem létezik, mivel a páratlan fokszámú csúcsok száma nem páros (az `1`-es fokszám miatt).
